...
def diagnose_direction(rows: list[dict[str, object]]) -> dict[str, object]:
    ...
    return {"current": current, "quantiles": quantiles}
...
def priority_score(
    ...
    if direction == "下调":
        gap = max(system_value - p95_value, 0.0) / max(abs(system_value), 1e-9)
    else:
        gap = max(p90_value - system_value, 0.0) / max(abs(p90_value), 1e-9)

    gap_score = min(gap / 0.30, 1.0)
    impact = min(abs(reduction_rate) / 0.50, 1.0)
    score = (0.45 * gap_score + 0.35 * confidence + 0.20 * impact) * 100
    score = round(score, 1)
    if score >= 75:
        return score, "A-优先"
    if score >= 55:
...
def run_diagnosis(
    ...
    for key, rows in child_groups.items():
        ...
        parent_system_mean = system_mean(parent_rows)
        if independent and parent_system_mean > p95_value:
            direction = "下调"
            selected_result = p95_result
        elif independent and parent_system_mean < p90_value:
            direction = "上调"
            selected_result = p90_result
        else:
            direction = "维持"
            selected_result = current
        selected_reduction_rate = (
            (float(current["positive_surplus"]) - float(selected_result["positive_surplus"]))
        ...
        records.append(
            {
                ...
                "调整判断": direction,
                "判断来源": "镀种子组独立判断" if independent else "样本不足_维持现状",
                "优先级得分": score,
                "优先级": level,
...
def main() -> None:
    ...
    payload = {
        "meta": {
            ...
            "调整判断": "样本量达到门槛后，父组系统均值>P95建议下调，父组系统均值<P90建议上调，位于P90-P95之间或样本不足时维持",
        },
